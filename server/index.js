            ...
            if (!!error) {
                ...
                connection.query(sql_query, [quantity, userData.ticketsToBuy], function(error, rows, fields) {
                    if (!!error) {
                        ...
                        sql_query = "SELECT Location, DepartTime, SchedDate FROM `train_schedule` TS JOIN `station_info` SI ON TS.CurrentStation = SI.StationID OR TS.Destination = SI.StationID WHERE TrainSchedID = ?";

                        connection.query(sql_query, userData.ticketsToBuy, function(error, rows, fields) {
                            if (!!error) {
                                console.log('Error in the query ' + error);
                            } else {
                                resp.send([fname, lname, rows[0].Location, rows[1].Location, rows[0].SchedDate, rows[0].DepartTime, quantity]);
                            }
                        });
                    }
                });    
...
app.get('/getScheduleFrom', function(req, resp) {
    ...
    const urlParams = new URLSearchParams(queryUrl);
    const location = urlParams.get('location');
    var sql_query = "SELECT TS.TrainID, TrainName, SI.Location, EI.EmpFName, EI.EmpLName, SchedDate, ArriveTime FROM `train_schedule` TS JOIN `train_info` TI ON TS.TrainID = TI.TrainID JOIN `employee_info` EI ON EI.EmpID = TS.EmpID JOIN `station_info` SI ON SI.StationID = TS.Destination WHERE TS.CurrentStation = (SELECT StationID FROM `station_info` SI WHERE SI.Location = ?)";
    var idx;
...
app.get('/purchaseTicket', function(req, resp) {
    ...
    var quantity = resp.req.query.quantity;
    ...
    function processReq(userData) {
        ...
        fs.writeFileSync('user-config.json', data);
        // ToDO: 
        // [ ] - Add to purchased_tickets
        // [ ] - 
        // [ ] - Update to ticket_schedule / Substract TicketsAvail
        // [ ] - PT - PassID, TrainSchedID, NumTickets
        resp.send(true);       
    }
})
...
app.get('/returnTicket', function(req, resp) {
    ...
    var sql_query = "DELETE FROM `purchased_tickets` WHERE PassID = ? AND TrainSchedID = ?;";

    connection.query(sql_query, [resp.req.query.passID, resp.req.query.ticketID], function(error, rows, fields) {
        if (!!error) {
            console.log('Error in the query ' + error);
        } else {
            var sql_query = "UPDATE `train_schedule` SET TicketsAvail = TicketsAvail + ? WHERE TrainSchedID = ?";

            connection.query(sql_query, [resp.req.query.quantity, resp.req.query.ticketID], function(error, rows, fields) {
                if (!!error) {
                    console.log('Error in the query ' + error);
                } else {
                    console.log(rows);
